...
# start=`date +%s`
# circom "$TEST_DIR"/circuits/test_batch_ecdsa_verify_"${1}".circom --r1cs --sym --c --output "$BUILD_DIR"
# end=`date +%s`
...
# start=`date +%s`
# $NODE_PATH --trace-gc --trace-gc-ignore-scavenger --max-old-space-size=2048000 --initial-old-space-size=2048000 --no-global-gc-scheduling --no-incremental-marking --max-semi-space-size=1024 --initial-heap-size=2048000 --expose-gc $SNARKJS_PATH zkey new "$BUILD_DIR"/"$CIRCUIT_NAME".r1cs "$PHASE1" "$OUTPUT_DIR"/"$CIRCUIT_NAME"_p1.zkey -v
# end=`date +%s`
# echo "DONE ($((end-start))s)"

# echo "****CONTRIBUTE TO PHASE 2 CEREMONY****"
# start=`date +%s`
# $NODE_PATH $SNARKJS_PATH zkey contribute -verbose "$OUTPUT_DIR"/"$CIRCUIT_NAME"_p1.zkey "$OUTPUT_DIR"/"$CIRCUIT_NAME"_p2.zkey -n="First phase2 contribution" -e="some random text for entropy"
# end=`date +%s`
# echo "DONE ($((end-start))s)"
...
# echo "****EXPORTING VKEY****"
# start=`date +%s`
# $NODE_PATH $SNARKJS_PATH zkey export verificationkey "$OUTPUT_DIR"/"$CIRCUIT_NAME"_p2.zkey "$OUTPUT_DIR"/"$CIRCUIT_NAME"_vkey.json -v
# end=`date +%s`
# echo "DONE ($((end-start))s)"
...
# start=`date +%s`
# $NODE_PATH $SNARKJS_PATH groth16 verify "$OUTPUT_DIR"/"$CIRCUIT_NAME"_vkey.json "$OUTPUT_DIR"/"$CIRCUIT_NAME"_public.json "$OUTPUT_DIR"/"$CIRCUIT_NAME"_proof.json -v
# end=`date +%s`
# echo "DONE ($((end-start))s)"