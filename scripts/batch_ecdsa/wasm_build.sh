...
start=`date +%s`
echo "$TEST_DIR"/"$CIRCUIT_NAME".circom
circom "$TEST_DIR"/"$CIRCUIT_NAME".circom --sym --wasm --output "$BUILD_DIR"
end=`date +%s`
echo "DONE ($((end-start))s)"

echo "****Executing witness generation****"
start=`date +%s`
node "$OUTPUT_DIR"/generate_witness.js "$OUTPUT_DIR"/"$CIRCUIT_NAME".wasm "$TEST_DIR"/input_"${1}".json "$OUTPUT_DIR"/witness.wtns
end=`date +%s`
echo "DONE ($((end-start))s)"
